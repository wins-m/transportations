...
def main():
    _PATH = '/Users/winston/mygitrep/transportations/'
    os.chdir(_PATH)
    
    # Define the source file path
    src = './data/transport_record.xlsx'
    ...
    tgt = filename_check('./configs/locCoords', 'json', fu=True)
    gen_loc_coords(spots, tgt)
    
    # Generate string of travel segments 
    tgt = filename_check('./configs/travelSegments', 'json', fu=True)
...
def cache_coords(sr: pd.Series) -> dict:
    ...
    for x0, ref in zip(sr['乘坐区间'].split('-'), [sr['始发站'], sr['终到站']]):
        ...
        if x1 not in head:
            API_KEY = "f5f19b61c04c94bb526e361bfa112afb"  # TODO: Replace with your Amap API key
